		...
		.addFields(
			...
			},
			{
				...
				value: [
					...
					"`/furniture info (id) (page)` - View info about furniture",
					"`/furniture sell <id>` - Bye bye furniture, hello money!",
					"`/furniture reclaim <id>` - Remove a piece of furniture from a house.",
				].join("\n"),
			},
		),
};
...
export const help: LifebotCommand = {
	...
	handler: async ({ interaction }) => {
		...
		console.error(subCommandName);
		await interaction.reply(
			"Something went wrong. This is our fault, an error message has been sent.",
		);
	},
};
